...
// TODO: Modify this code to wrap it for VC++
//#define CU8(...) (ARRAY(const uint8_t, __VA_ARGS__))
#define CU8(...) {__VA_ARGS__}
...
static void SPI_send(uint8_t cs_pin, std::initializer_list<uint8_t> buffer, uint16_t length);
static uint8_t SPI_read(uint8_t cs_pin, std::initializer_list<uint8_t> buffer, uint16_t length);
...
void EPD_Class::begin(void) {
	...
	// TODO: Modify this code to wrap appropriately for VC++ and to use the initializer list
	//SPI_send(this->EPD_Pin_EPD_CS, this->channel_select, this->channel_select_length);
	SPI_send(this->EPD_Pin_EPD_CS, { 0x72, 0x00, 0x00, 0x00, 0x7f, 0xff, 0xfe, 0x00, 0x00 }, this->channel_select_length);
...
void EPD_Class::line(uint16_t line, const uint8_t *data, uint8_t fixed_value,
       ...
       if (set_voltage_limit) {
	       ...
	       SPI_send(this->EPD_Pin_EPD_CS, CU8(0x70, 0x04), 2);
		   // TODO: Fix this to work with the initializer list
	       const uint8_t b[2] = {0x72, this->voltage_level};
		   SPI_send(this->EPD_Pin_EPD_CS, { 0x72, 0x00 }, sizeof(b));
       }
...
// TODO: Seeing if bring the SPI interface up and down is the perf issue
//static void SPI_on(void) {
//	SPI.end();
//	SPI.begin();
...
// TODO: Need to modify this to wrap for VC++
//static void SPI_send(uint8_t cs_pin, const uint8_t *buffer, uint16_t length) {
//	// CS low
...
//}
static void SPI_send(uint8_t cs_pin, std::initializer_list<uint8_t> buffer, uint16_t length) {
	// CS low { 0x72, 0x00, 0x00, 0x00, 0x7f, 0xff, 0xfe, 0x00, 0x00 }
	digitalWrite(cs_pin, LOW);
